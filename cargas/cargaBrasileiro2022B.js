...
const cadastrarDados = async () => {
    ...
    const campeonato = await Campeonato.findOne({codigo: "BRA2022B"});
    
    const arquivo = "brasileiroserieb2022.txt";
    ...
    for(var i = 0; i < listaPartidas.length; i++) {
        ...
        if(linhaInformativaDeRodada(i+1)) {
            console.log("[" + i + "] = " + partidas.length + "partidas para rodada " + rodada)
